...
const Therapies = () => {
  ...
  // Form states — blank by default, populated on Add or Edit
  ...
  const handleNameChange = (val) => {
    ...
    setFormData((prev) => ({ ...prev, therapy: val, slug: slugged }));
    if (errors.therapy) setErrors((prev) => ({ ...prev, therapy: null }));
  };

  // Filter list
  const filtered = useMemo(() => {
    return therapies.filter((item) =>
      item.therapy.toLowerCase().includes(search.toLowerCase()) ||
  ...
  // Table columns definition: THERAPY, SLUG, USED IN, PRESENCES, ACTIONS
  const columns = [
    {
      header: 'Therapy',
      accessor: 'therapy',
  ...
  const activeIconPreset = iconPresets.find((ip) => ip.name === formData.icon) || iconPresets[0];
        ...
        {editorOpen && (
          <div className="rounded-3xl border border-white/[0.04] bg-surface lg:col-span-1 p-3 shadow-soft flex flex-col min-h-0 overflow-hidden">
            ...
            <div className="flex items-center justify-between pb-1.5 border-b border-white/[0.04] mb-2 shrink-0">
              <div>
                <h3 className="text-sm font-bold text-white">Create / Edit Therapy</h3>
              </div>
              <button
                type="button"
            ...
            <div className="space-y-1 flex-1 min-h-0 pr-1 overflow-hidden">
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  <label className="text-xs font-semibold text-textSecondary">
                    Short Name (for badges) <span className="text-rose-500">*</span>
                  </label>
                ...
                <input
                  ...
                  className={`w-full rounded-xl border ${errors.shortName ? 'border-rose-500' : 'border-white/[0.04]'} bg-white/5 px-3.5 py-1.5 text-sm text-white outline-none transition focus:border-[#3B5BFF]`}
                />
              ...
              {/* Status and Live Preview Grouped Closely */}
              <div className="space-y-1 pt-1 border-t border-white/[0.04]">
                ...
                <div>
                  <p className="text-[11px] font-semibold text-textSecondary mb-1.5">Preview (How it will appear on frontend)</p>
                  <div className={`rounded-2xl border ${formData.status === 'Active' ? 'border-[#27D4A0]/20 bg-[#27D4A0]/5' : 'border-white/[0.04] bg-white/[0.02]'} p-2 transition-all duration-300`}>
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-center gap-2">
                        <div className={`flex h-8 w-8 items-center justify-center rounded-xl border ${activeIconPreset.bg} ${activeIconPreset.text} ${activeIconPreset.border}`}>
                          <ActiveIcon size={14} />
                        </div>
                        <h4 className="text-[12px] font-bold text-white leading-snug">{formData.therapy || 'Therapy Title'}</h4>
                      </div>

                      <div className="text-right flex flex-col items-end shrink-0">
                        <span className="text-base font-bold text-[#27D4A0] leading-none">
                          {formData.presences || 0}
                        </span>
                        <span className="text-[8.5px] text-textSecondary uppercase font-bold tracking-wider leading-none mt-0.5">
                          Presences
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="mt-2 flex flex-wrap gap-1.5 pt-1.5 border-t border-white/[0.04]">
                      <span className="rounded bg-[#27D4A0]/10 border border-[#27D4A0]/20 px-1 py-0.5 text-[8.5px] text-[#27D4A0] font-semibold">{formData.shortName || 'Badge 1'}</span>
                      <span className="rounded bg-[#06B6D4]/10 border border-[#06B6D4]/20 px-1 py-0.5 text-[8.5px] text-[#06B6D4] font-semibold">Gastro</span>
                      <span className="rounded bg-[#a855f7]/10 border border-[#a855f7]/20 px-1 py-0.5 text-[8.5px] text-[#a855f7] font-semibold">Pediatric</span>
                      <span className="rounded bg-white/5 border border-white/[0.04] px-1 py-0.5 text-[8.5px] text-textSecondary font-semibold">+17</span>
                    </div>
                  </div>
                </div>
              </div>
